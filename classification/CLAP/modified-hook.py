...
class CLAP_Module(torch.nn.Module):
    ...
    def load_ckpt(self, ckpt = None, model_id = -1, finetune_ckpt = None):
        ...
        if finetune_ckpt:
            finetune_ckpt = load_state_dict(finetune_ckpt, skip_params=True)
            for key in list(finetune_ckpt.keys()):
                    if 'clap_model.' in key:
                        fixed_key = key.replace('clap_model.', '')
                        finetune_ckpt[fixed_key] = finetune_ckpt[key]
                        del finetune_ckpt[key]
                    ckpt[fixed_key] = finetune_ckpt[fixed_key]
